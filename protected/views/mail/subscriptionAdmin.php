					...
					<tbody>
						<tr>
							<td style="font-family:Roboto,Tahoma,Verdana,Segoe,sans-serif; color: #555; font-size: 14px;" valign="top">
								<div>
									<p style="font-size:34px;color:rgb(57,150,220)">Notifica Pagamento</p>
								</div>

								<div>
									<p style="font-size: 14px;">Pagamento ricevuto con transazione n: <?php echo $id_invoice_bps; ?>
									</p>
								</div>
								<div>
									<h3 style="margin-top: 10px;">Emessa ricevuta n: <?php echo $progressivo.'/'.$anno;?> del <?php echo WebApp::data_it($data_registrazione);?></h3>
								</div>
								<div>
										<p style="padding: 3px 3px 3px 3px;
											...
											<br>
											Socio: <?php echo $name.' '.$surname; ?><br>
								        	Importo: € <?php echo $importo; ?><br>
											Tipo quota: <?php echo Quote::model()->findByPk($id_quota)->description; ?><br>
											Validità dal: <?php echo WebApp::data_it($data_inizio); ?> al: <?php echo WebApp::data_it($data_scadenza); ?><br>
											<br>
											Modalità di pagamento: <?php echo TipoPagamenti::model()->findByPk($id_tipo_pagamento)->description; ?>
