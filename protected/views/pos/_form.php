...
$ajaxSelectStores = Yii::app()->createUrl('pos/ajaxCreateSelectStore');
$script = <<<JS
	$("#Pos_id_merchant").change(function() {
		...
		$.ajax({
			...
			success: function(result) {
				//console.log(result);
				var my_list = $("#Pos_id_store").empty();
...
?>
	...
	<?php if (Yii::app()->user->objUser['privilegi'] == 20){ ?>
		...
		<div class="form-group" id='hiddenStores' style="display:<?php echo $display;?>">
			...
			<!-- <select class='form-control' name='Pos[id_store]' id='Pos_id_store'></select> -->
	...
	<div class="form-group">
		<?php echo $form->labelEx($model,'Descrizione POS'); ?>
		...
		<p class="text-primary"><i>Descrivere il <b>POS</b> secondo l'organizzazione interna (es. POS principale, POS piano terra, POS reparto 1, ecc…)</i></p>
	</div>
